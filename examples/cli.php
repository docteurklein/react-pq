...
$pq = new \ReactPq\Client($loop, new \ReactPq\ConnectionPool('host=postgres user=postgres'));
$loop->addPeriodicTimer(1, function() use($pq, $loop, $stdout) {
    $stream = $pq->query("select now(), s from generate_series(0, 100000) s", [], [], function($data) {
